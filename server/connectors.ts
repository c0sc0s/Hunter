...
export const connectorDefinitions: ConnectorDefinition[] = [
  {
    ...
    capabilities: [
      ...
      "Legacy doc raw-content import",
      "Access-token refresh before sync",
      "Permission-aware refresh (next)"
    ],
    setupMessage:
      "OAuth, encrypted token refresh, and manual docx/legacy-doc/wiki raw-content import are available when Feishu app credentials are configured. Browser snapshot capture remains the fallback for unsupported Feishu surfaces."
  },
  {
    provider: "x",
    label: "X",
