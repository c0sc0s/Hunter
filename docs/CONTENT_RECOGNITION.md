...
- Feishu/Lark: browser snapshot can produce `ready` when substantial visible content is captured, or `partial` when limited; OAuth authorization, sync-time access-token refresh, direct docx, legacy doc, and wiki-node-to-docx/doc raw-content import are available, while non-document wiki import and full native block fidelity remain future connector work.
